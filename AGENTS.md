...
- **活跃门槛用中位数不用平均数**（2026-09-09）：发过言成员发言数的中位数（向上取整），0 条者不参与计算。群聊发言是长尾分布（实测前 10 人占 71%），平均数会被头部拉到 795 条、仅 16% 达线，且去掉一人就变 645；中位数对分布形状不敏感，长尾群、均匀群、潜水群都稳定给出“发过言者的上半”，换群不需调参。它是相对口径（永远恰好一半人活跃），所以页面上始终把门槛数字写在“活跃成员”旁边。要换口径只改 `activeThresholdOf()`
